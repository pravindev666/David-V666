...
def engineer_features(df, target_horizon=5):
    ...
    # NOTE: is_expiry_week removed — NSE expiry day changed multiple times
    # (Fri→Thu→Tue), hardcoding any day is wrong for historical data.
    # day_of_week already captures day-of-week effects.
    
    # ═══════════════════════════════════════════════════════════════════════
    # 9. MEAN-REVERSION / REVERSAL FEATURES (5 features)
    # These help catch trend turns (model's biggest weakness)
    # ═══════════════════════════════════════════════════════════════════════
    # How far from recent high/low — stretched = likely to snap back
    df["dist_20d_high"] = (df["close"] - df["high"].rolling(20).max()) / df["close"]
    df["dist_20d_low"] = (df["close"] - df["low"].rolling(20).min()) / df["close"]
    
    # RSI divergence: price drops but RSI doesn't → hidden bullish signal
    df["rsi_divergence"] = df["rsi_14"].diff(5) - (df["returns_5d"] * 100)
    
    # Consecutive down/up days (5-day window) — extreme = mean reversion
    df["consec_down_5"] = (df["returns_1d"] < 0).rolling(5).sum()
    df["consec_up_5"] = (df["returns_1d"] > 0).rolling(5).sum()
    
    # ═══════════════════════════════════════════════════════════════════════
    # 10. VOLUME FEATURES (2 features)
    # ═══════════════════════════════════════════════════════════════════════
