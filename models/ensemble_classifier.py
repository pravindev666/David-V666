...
class EnsembleClassifier:
    ...
    def train(self, df, feature_cols, verbose=True):
        ...
        # Scaler is fit INSIDE each fold to prevent future data leakage
        ...
        # Refit scaler on full dataset for production use
    ...
    def predict(self, X_row):
        ...
        for i in range(len(combined_probs)):
            ...
            # SIDEWAYS override: if model isn't confident, don't make a directional bet
            from utils import SIDEWAYS_CONFIDENCE_THRESHOLD
            if confidence < SIDEWAYS_CONFIDENCE_THRESHOLD and direction != SIDEWAYS:
                direction = SIDEWAYS
                confidence = float(prob[2])  # use actual SIDEWAYS probability (honest)
    ...
    def detailed_backtest(self, df, feature_cols, train_end_year=2023):
        ...
        Uses performance-based model weights (matching production behavior).
        """
        ...
        # ─── Derive performance weights via CV on training data ────────
        tscv = TimeSeriesSplit(n_splits=3)
        cv_scores = {name: [] for name in self._build_models().keys()}
        
        for train_idx, val_idx in tscv.split(X_train):
            cv_scaler = StandardScaler()
            X_cv_train = cv_scaler.fit_transform(X_train[train_idx])
            X_cv_val = cv_scaler.transform(X_train[val_idx])
            y_cv_train, y_cv_val = y_train[train_idx], y_train[val_idx]
            
            cv_models = self._build_models()
            for name, model in cv_models.items():
                model.fit(X_cv_train, y_cv_train)
                acc_cv = accuracy_score(y_cv_val, model.predict(X_cv_val))
                cv_scores[name].append(acc_cv)
        
        bt_weights = {}
        avg_scores = {name: np.mean(scores) for name, scores in cv_scores.items()}
        total_score = sum(avg_scores.values())
        for name in avg_scores:
            bt_weights[name] = avg_scores[name] / total_score
        
        print(f"\n  {C.CYAN}Backtest Model Weights (from training CV):{C.RESET}")
        for name, w in bt_weights.items():
            print(f"    {name:>10}: {w:.3f} (CV acc: {avg_scores[name]:.1%})")
        ...
        # ─── Test on unseen data with performance weights ─────────────
        ...
        for name, model in temp_models.items():
            ...
            combined_probs += probs * bt_weights[name]
